      ...
      <div className="max-w-2xl mx-auto px-4 py-8">
        <div className="bg-white dark:bg-gray-900 rounded-xl shadow-sm p-6">
          ...
          <div className="grid grid-cols-2 gap-4 mb-4">
            <div>
              <label htmlFor="maker" className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Maker</label>
              <input id="maker" type="text" value={maker} onChange={e => setMaker(e.target.value)}
          ...
          <div className="grid grid-cols-3 gap-4 mb-6">
            <div>
              ...
              <select id="province" title="Province" value={province}
                onChange={e => { setProvince(e.target.value); setCanton(''); setDistrict('') }}
                className="w-full border border-gray-300 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-100 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
                <option value="">Select...</option>
                {locations.map((p: Province) => <option key={p.name} value={p.name}>{p.name}</option>)}
              </select>
            </div>
            <div>
              <label htmlFor="canton" className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Canton</label>
              <select id="canton" title="Canton" value={canton}
                onChange={e => { setCanton(e.target.value); setDistrict('') }}
                disabled={!province}
                className="w-full border border-gray-300 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-100 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 disabled:bg-gray-100 dark:disabled:bg-gray-700">
                <option value="">Select...</option>
                {selectedProvince?.cantons.map((c: Canton) => <option key={c.name} value={c.name}>{c.name}</option>)}
              </select>
            </div>
            <div>
              <label htmlFor="district" className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">District</label>
              <select id="district" title="District" value={district}
                onChange={e => setDistrict(e.target.value)}
                disabled={!canton}
                className="w-full border border-gray-300 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-100 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 disabled:bg-gray-100 dark:disabled:bg-gray-700">
                <option value="">Select...</option>
                {selectedCanton?.districts.map((d: string) => <option key={d} value={d}>{d}</option>)}
              </select>
            </div>
          </div>
          ...
          <div className="mb-6">
            ...
            <input id="photos" type="file" multiple accept="image/jpeg,image/png"
              title="Upload car photos" onChange={handlePhotos}
              className="w-full text-sm text-gray-500 dark:text-gray-400 file:mr-4 file:py-2 file:px-4 file:rounded-lg file:border-0 file:bg-blue-50 file:text-blue-700 hover:file:bg-blue-100" />
            {previews.length > 0 && (
